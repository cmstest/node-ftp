    ...
    function reentry(err, text, code) {
      ...
      if (!cmd) {
        ...
        if (!err)
          ...
          if(self._feat.toString().toLowerCase().indexOf("pret") > -1) {
            self._pret = true;
          }
        cmd = 'TYPE';
...
FTP.prototype.list = function(path, zcomp, cb) {
  ...
  this._pasv(cmd, function(err, sock) {
    if (err)
      return cb(err);
...
FTP.prototype._pasv = function(cmd,cb) {
  ...
  this._send(cmd, function(){


  self._send('PASV', function reentry(err, text) {
    if (err)
      return cb(err);
...
FTP.prototype._store = function(cmd, input, zcomp, cb) {
  ...
  if(cmd.indexOf("APPE") > -1 || cmd.indexOf("STOR") > -1)
    cmdPret = cmd;

  this._pasv(cmdPret, function(err, sock) {
    if (err)
      return cb(err);
